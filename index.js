  ...
  `,
    ...
    return new Promise((resolve) => {
        ...
        client.once("message", (wsMsg) => {
            ...
            if (rc == 12) {
                console.log(`Successfully claimed chest for Account ${accountID} got => ${data.number} ${data.type == 1 ? "MTB" : "TON"}`);
            }
      ...
      if (event.userData != undefined) {
        ...
        if (event.bossInfo != undefined) {
          ...
          if (event.bossInfo.remain != 0) {
            console.log("Account " + accountID + " In cooldown for " + millisecondsToHoursAndMinutes(event.bossInfo.remain));

            // Update account status to true
            accountList[idx][1] = true;
            var nextIdx = accountList.findIndex((account) => account[1] === false);
